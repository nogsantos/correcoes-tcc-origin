...
A mineração de dados, em inglês \textit{data mine} sigla \textit{DM}, é um processo de busca em grandes massas de dados por padrões que possam ser utilizados para a geração de conhecimento, entretanto ele por si só não pode ser definido como responsável pelo processo de geração deste conhecimento. O processo como um todo é conhecido como descoberta de conhecimento em banco de dados, em inglês \textit{knowledge discovery in database} sigla \textit{KDD}.
...
\begin{flushleft}
\textbf{São alguns exemplos de mineração de dados}
\end{flushleft}
